...
local storage = 20000
local t = {
[1] = {keyword = "hora", text = "Você tera que matar o general Gord! Cuidado ele é muito poderoso, e nossa {causa} depende disso!"}, -- palavra-chave, texto
    ...
    if(not npcHandler:isFocused(cid)) then
        if isInArray({"hi", "hello","ola","oi"}, msg) then
            if (getPlayerStorageValue(cid, storage) == npcOrder) or getPlayerStorageValue(cid, storage) == npcOrder then
                ...
                npcHandler:say("Humm, então chegou a {hora}?! ", cid)
