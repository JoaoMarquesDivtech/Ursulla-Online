...
function onGetFormulaValues(player, skill, attack, factor)
	local min = (player:getLevel() / 5) + (skill + attack)*0.5   + (FORCA(player)  * 2.7)
    return -min, -min
end
...
function onGetFormulaValues(player, skill, attack, factor)
	local min = ((player:getLevel() / 5) + (skill * attack * 0.008)  + (FORCA(player) * 2.5))*0.3
    return -min, -min
end
...
function onCastSpell(player, variant)
	...
	if not Cooldown(player,"Ceifer Cut",30000,5) then return false end
