local config = {
	...
	[7588] = {health = {min = 250, max = 350}, level = 50, emptyId = 7634},
	-- strong mana potion
	[7589] = {mana = {min = 115, max = 185}, level = 50, emptyId = 7634},
	-- great mana potion
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if (potion.level and player:getLevel() < potion.level)
		...
		player:say(string.format('Você precisa de um nivel superior para usar essa porção.', potion.text, potion.level), TALKTYPE_MONSTER_SAY)
		return true
	end
	...
	if topParent.isItem and (not topParent:isItem() or topParent.itemid ~= 460) then
	        ...
	        parent:addItem(potion.emptyId, 1)
 			item:remove(1)
			return true
		--end
	end
