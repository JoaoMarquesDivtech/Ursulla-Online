...
local talkUser, msg = NPCHANDLER_CONVbehavior == CONVERSATION_DEFAULT and 0 or cid, string.lower(msg)
    ...
    elseif msgcontains(msg, "Luz") then
            npcHandler:say("Voce virou um {Druida da Luz}! O iluminador! Normalmente se usa Varinha Magica!", cid)
            setPlayerStorageValue(cid, storage, npcOrder + 1)
			doPlayerSetVocation(cid,23)
			npcHandler:releaseFocus(cid)
	elseif msgcontains(msg, "Natureza") then
            npcHandler:say("Voce virou um {Druida da Natureza}! O grande amigo da natureza, normalmente se usam varinhas magicas!", cid)
            setPlayerStorageValue(cid, storage, npcOrder + 1)
