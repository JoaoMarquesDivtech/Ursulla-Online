...
local talkUser, msg = NPCHANDLER_CONVbehavior == CONVERSATION_DEFAULT and 0 or cid, string.lower(msg)
	...
	elseif  msgcontains(msg, "Trevas") then
            npcHandler:say("Voce virou um {Mago das Trevas}! O seu maior dom é que a morte não é um impedimento! Essa classe normalmente usa varinhas como arma!", cid)
            setPlayerStorageValue(cid, storage, npcOrder + 1)
			doPlayerSetVocation(cid,16)		
			npcHandler:releaseFocus(cid)			
   elseif  msgcontains(msg, "Raio") then
            npcHandler:say("Você virou um {Mago da Raio}! O som dos trovões é apenas um suspiro seu, Essa classe normalmente usa varinhas como arma!", cid)
            setPlayerStorageValue(cid, storage, npcOrder + 1)
			doPlayerSetVocation(cid,20)
			npcHandler:releaseFocus(cid)
   elseif  msgcontains(msg, "Gelo") then
            npcHandler:say("Você virou um {Mago da Gelo! A nevasca é um refugio de férias! Essa classe normalmente usa varinhas como arma!", cid)
            setPlayerStorageValue(cid, storage, npcOrder + 1)
